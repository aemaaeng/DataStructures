...
class HashChaining:
    ...
    def set(self, key, value=None):  # (key, value) 쌍을 해당 슬롯의 연결리스트의 가장 앞에 삽입한다. (return 값 없음)
    ...
    def remove(self, key):  # key값을 갖는 노드를 찾아 제거한다. 해당 노드가 없다면 None, 있다면 key
        i = self.find_slot(key)
        v = self.H[i].search(key)
        pop = self.H[i].remove(v)
        if pop == None:
            return None
        else:
            return key
